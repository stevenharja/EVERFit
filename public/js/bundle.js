...
exports.clearLoader = exports.renderLoader = exports.elementStrings = exports.formElement = exports.popup = exports.elements = void 0;
var elements = {
...
var renderLoader = function renderLoader(parent) {
  var beginAt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'afterbegin';
...
},{"axios":"../../node_modules/axios/index.js"}],"changeProfile.js":[function(require,module,exports) {
"use strict";
...
var changeProfile = /*#__PURE__*/function () {
  ...
  return function changeProfile(_x, _x2) {
    return _ref.apply(this, arguments);
  };
}();

exports.changeProfile = changeProfile;
},{"axios":"../../node_modules/axios/index.js"}],"editModel.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.editModel = void 0;
...
var editModel = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(model, id, data) {
    var url, res;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = "/api/v1/".concat(model, "/").concat(id);
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
          ...
          case 8:
            _context.prev = 8;
            _context.t0 = _context["catch"](0);
            alert("".concat(model, " failed to be updated. Please ensure that you are authorized to modify this ").concat(model));
  ...
  return function editModel(_x, _x2, _x3) {
    return _ref.apply(this, arguments);
  };
}();
...
var _changeProfile = require("./changeProfile");

var _editModel = require("./editModel");
...
if (_base.elements.userDataForm) {
  _base.elements.userDataForm.addEventListener('submit', function (e) {
    e.preventDefault();
    ...
    (0, _changeProfile.changeProfile)(form, 'data');
  });
}

if (_base.elements.userPasswordForm) {
  _base.elements.userPasswordForm.addEventListener('submit', /*#__PURE__*/function () {
    var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(e) {
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              e.preventDefault(); // document.querySelector('.btn-password').textContent = 'Updating...';
              ...
              _context.next = 6;
              return (0, _changeProfile.changeProfile)({
...
if (_base.elements.editModelForm) {
  console.log('Edit form exists');

  _base.elements.editModelForm.addEventListener('submit', function (e) {
    e.preventDefault();
    var pathnames = window.location.pathname.split('/');
    var model = pathnames[2];
    var id = pathnames[3];
    var form = new FormData();
    if (_base.formElement.name) form.append('name', _base.formElement.name.value);
    if (_base.formElement.summary) form.append('summary', _base.formElement.summary.value);
    if (_base.formElement.category) form.append('category', _base.formElement.category.value);
    if (_base.formElement.imageCover) form.append('imageCover', _base.formElement.imageCover.files[0]);
    if (_base.formElement.description) form.append('description', _base.formElement.description.value);
    (0, _editModel.editModel)(model, id, form);
  });
} //////FRONT END////////


if (_base.elements.navbar) {
  ...
  var sticky = _base.elements.navbar.offsetTop; // Add the sticky class to the navbar when you reach its scroll position. Remove "sticky" when you leave the scroll position

  var stickyHeader = function stickyHeader() {
    if (window.pageYOffset >= sticky) {
      _base.elements.navbar.classList.add('sticky');
    } else {
      _base.elements.navbar.classList.remove('sticky');
    }
  };
}

window.addEventListener('click', function (e) {
  if (e.target === _base.elements.popup || e.target.closest('.popup__button--close')) {
    _base.elements.popup.style.display = 'none';

    _base.elements.body.classList.toggle('show-popup');

    _base.popup.content.innerHTML = '';
  }
});

if (_base.elements.activityItems.length > 0) {
  console.log('There is activity item here.');

  var modifyPopup = function modifyPopup(data) {
    // renderLoader(popup.box);
    _base.popup.content.innerHTML = '';
    var imageMarkup = "<img src=\"".concat(data[0].src, "\" alt=\"").concat(data[0].alt, "\" class=\"popup__media\" />");
    var textMarkup = "<div class=\"popup__text\">\n                          <div class=\"popup__title\">".concat(data[1].innerText, "</div>\n                          <div class=\"popup__summary\">").concat(data[2].innerText, "</div>\n                        </div>");
    var descriptionMarkup = "<div class=\"popup__description\">\n    Lorem ipsum dolor sit amet consectetur adipisicing elit. Porro vel\n    eius, saepe necessitatibus dolor atque doloribus corporis voluptatem\n    neque in? Lorem ipsum dolor sit amet consectetur adipisicing elit.\n    Cupiditate repellat aut atque ad aliquam quaerat rem facilis minima\n    eligendi fugit, explicabo officia voluptates eveniet dolorem\n    obcaecati soluta. Dolore molestias animi quis quia, est temporibus\n    quam beatae harum placeat magni ipsa. </div>";
    var fullMarkup = "".concat(imageMarkup, " ").concat(textMarkup, " ").concat(descriptionMarkup);

    _base.popup.content.insertAdjacentHTML('afterbegin', fullMarkup); // clearLoader();

  };

  var showContent = function showContent(n) {
    return function () {
      modifyPopup(_base.elements.activityItems[n].children); //If it is at the start of the array.

      if (n == 0) {
        _base.popup.leftButton.style.display = 'none';
      } else {
        _base.popup.leftButton.style.display = 'block';
        _base.popup.leftButton.onclick = showContent(n - 1);
      } //If it is at the end of array.


      if (n >= _base.elements.activityItems.length - 1) {
        _base.popup.rightButton.style.display = 'none';
      } else {
        _base.popup.rightButton.style.display = 'block';
        _base.popup.rightButton.onclick = showContent(n + 1);
      }
    };
  };

  _base.elements.activityItems.forEach(function (item, index) {
    item.dataset.id = index;
    item.addEventListener('click', function () {
      _base.elements.popup.style.display = 'block';
...
if (_base.elements.dropdownBoxes.length > 0) {
  console.log('There is a dropdown box here.');

  _base.elements.dropdownBoxes.forEach(function (item, index) {
    item.addEventListener('click', function () {
      console.log('item clicked'); // item.lastElementChild.classList.toggle('display-content');
